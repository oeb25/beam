...
void main() {
    ...
    vec4 normalEmission = texture(aNormal, TexCoords);
    vec3 normal = normalize(normalEmission.rgb);
    ...
    for (int i = 0; i < nrPointLights; ++i) {
        color += calcPLight(pointLights[i], samples, viewDir, normal, fragPos);
    }
    for (int i = 0; i < nrDirLights; ++i) {
        color += calcDLight(directionalLights[i], samples, viewDir, normal, fragPos);
    }
